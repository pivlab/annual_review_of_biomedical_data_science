...
Although genome-wide association studies (GWAS) have identified numerous genetic variants associated with complex traits, translating these findings into biological insights remains challenging [@doi:10.1038/s41576-019-0127-1].
...
A simple strategy to link GWAS-associated variants with a gene is by physical proximity, which typically selects the closest gene to the most significant single nucleotide polymorphism (SNP) at each locus.
...
The paragraph is well-written and clear, but here are a few minor adjustments for improved clarity and readability:
...
No significant errors were found, but the text can be slightly more concise by removing redundant phrases.

The paragraph you provided is well-written and clear.
Here is a slightly refined version with minimal changes for clarity and flow:
...
All of these methods implement a similar framework consisting of three steps: 1) model training, 2) gene expression imputation, and 3) gene-trait association.
...
In step 3, a gene-tissue-trait association is computed by correlating the tissue-specific imputed gene expression with the trait of interest.
Most methods, such as Summary-PrediXcan [@doi:10.1038/s41467-018-03621-1] or S-PrediXcan, offer a shortcut by computing a gene-tissue-trait association directly from GWAS summary statistics without the need for individual-level data.
...
The paragraph you provided is already well-written and only requires minimal adjustments for clarity and flow.
...
No grammatical errors were found, and the content is coherent and well-structured.
...
For instance, a study by Shen et al.
(2022) demonstrated a causal relationship between DNA methylation and depression, indicating that epigenetic modifications may mediate genetic risk for psychiatric disorders [@doi:10.1186/s13073-022-01039-5].
...
Del Rosario et al.
(2022) conducted a HAWAS, identifying over 2,000 differentially acetylated loci in immune cells from *Mycobacterium tuberculosis*-infected individuals, linking these changes to gene expression and potassium channel genes like *KCNJ15* in modulating apoptosis and Mtb clearance.
