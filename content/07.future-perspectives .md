...
Previous research has demonstrated significant speedups using GPU acceleration across various computational tasks: up to 37x for sequence alignment tools, 100x for molecular dynamics tools using multiple GPUs, and a maximum of 1000x for dynamic simulations [@doi:10.1093/bib/bbw058].
In our exploration, we experimented with several Python libraries that primarily utilize Nvidia's CUDA [@doi:10.1145/1401132.1401152] underneath for GPU computing.
CuPy[@url:https://scholar.google.co.jp/citations?view_op=view_citation&hl=ja&user=jBDgSvUAAAAJ&citation_for_view=jBDgSvUAAAAJ:W7OEmFMy1HYC] and libraries provided by project RAPIDS [@url:https://rapids.ai/], provide simple drop-in replacements of existing computation-bound code such as common function calls in Numpy [@doi:10.1038/s41586-020-2649-2] and Scikit-learn [@doi:10.48550/arXiv.1201.0490].
Using those counterparts and assessing whether the GPU acceleration provides sufficient performance improvement would be an easy first step to look into.
If more speedup is needed, the next stage would involve creating custom CUDA functions using any library that supports it, for instance, Cupy, to tailor the optimization to specific computational needs.
For more advanced features and fine-grained tuning, consider using PyCUDA [@doi:10.1016/j.parco.2011.09.001]. 
